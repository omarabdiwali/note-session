      ...
      <div className={`cursor-auto ${!open ? "hidden" : ""} z-50`}>
        <div className={`fixed flex h-screen inset-0 z-50 transition-all duration-300 delay-150 ease-in-out ${!open ? "opacity-0 hidden" : "opacity-100"} w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-96 max-h-full`}>
          <div className="relative m-auto w-full max-w-lg max-h-full">
            <div className="relative rounded-lg shadow bg-slate-900 text-white">
              <div className="text-2xl font-bold p-3 m-3">Delete Note</div>
              <div className="text-lg opacity-80 mx-5">
              ...
              <div className="flex justify-end p-6 space-x-2 rounded-b border-gray-600">
                ...
                <button disabled={disabled} className="disabled:opacity-75 focus:ring-4 focus:outline-none rounded-lg border text-sm font-medium px-5 py-2.5 hover:text-gray-900 focus:z-10 bg-gray-800 text-gray-300 border-gray-500 hover:text-white hover:bg-black focus:ring-gray-600" onClick={() => setOpen(false)}>Cancel</button>
              </div>
            </div>
          </div>
        </div>
